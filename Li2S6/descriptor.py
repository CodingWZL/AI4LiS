...
def adsorption_feature(ratio, flag):
    feature = np.array(())
    if flag == 0:
        for i in range(len(ratio)):
            if ratio[i] == 0:
                ratio[i] = 1
            feature =np.hstack((feature, feat_Li / (feat_S * ratio[i])))
    else:
        for i in range(len(ratio)):
            if ratio[i] == 0:
                ratio[i] = 1
            feature =np.hstack((feature, feat_S / (feat_Ni * ratio[i])))
    # print(feature)
    return feature
...
def calculate_feature(file_name):
    ...
    feature_ads_Ni_S = adsorption_feature(ratio_Ni, flag=1)
    feature_ads_S_Li = adsorption_feature(ratio_S, flag=0)

    # print(feature_com.shape, feature_str.shape, feature_ads.shape)
    # print(feature_com, feature_str, feature_ads)
    feature = np.concatenate((feature_com, feature_str_Ni, feature_str_S, feature_ads_Ni_S, feature_ads_S_Li), axis=0)
    return feature
...
def main():
    ...
    feature = feature.reshape((-1, 18))
    #np.savetxt("S6-feature.txt", feature)
